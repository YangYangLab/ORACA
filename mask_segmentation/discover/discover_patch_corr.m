function [segmap] = discover_patch_corr(patch,params)
%DISCOVER_PATCH_CORR calculates local correlation map for seed generation
%   SEEDRC = DISCOVER_PATCH_CORR(DATA, PARAMS) computes all pixels in DATA and gives out
...
patchsz = get_option(params, 'PatchSize', 5); 
...
if useGPU
    ...
    patch = double(patch);
    patch = (patch - mean(patch,3)) ./ std(patch,[],3);
    ...
    for r = 2:h-1
        along = repmat(patch(r,2:w-1,:), [9 1 1]);
        blong = [patch(r-1:r+1,1:w-2,:); patch(r-1:r+1,2:w-1,:); patch(r-1:r+1,3:w,:)];
