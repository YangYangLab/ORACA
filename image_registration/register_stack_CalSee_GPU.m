function [rt, stackOut] = register_stack_CalSee_GPU(moving, template, params)
%REGISTER_STACK_CALSEE_GPU fast image registration using GPU
%   RT = REGISTER_STACK_CALSEE_GPU(MOVING, PARAMS) analyses movements in the video 
%   and outputs a table comprising offsets in X-Y direction. 
%   [..., STACKOUT] = REGISTER_STACK_CALSEE_GPU() also outputs registered stack if needed.
% Inputs:
...
%
%   See also IM_CONVFFT, REGISTER_STACK_CALSEE_GPU.
...
elseif nargout == 2 % rt stackOut
    ...
    stackOut = gpuArray.zeros(size(moving), classUnderlying(moving));
